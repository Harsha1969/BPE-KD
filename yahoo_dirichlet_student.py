...
# ## Student Model(with dirichlet output) training and evaluation on Yahoo Answers polarity dataset
...
def yahoo_uncertainties(yahoo_alpha_test,epoch):
    yahoo_alpha_test = torch.load("yahoo_alpha_test.pt")
    ...
    }, f'yahoo_yahoo_uncertainties_seed{seed}_epoch{epoch}.pt')
...
def amazon_uncertainties(epoch):
    ...
    amazon_alpha_test = get_test_alpha(test_dataloader, classifier)
    stu_probs = dirichlet_to_prob(amazon_alpha_test)
    ...
    }, f'yahoo_amazon_uncertainties_seed{seed}_epoch{epoch}.pt')
...
def sst2_uncertainties(epoch):
    ...
    }, f'yahoo_sst2_uncertainties_seed{seed}_epoch{epoch}.pt')
...
def youtube_uncertainties(epoch):
    ...
    }, f'yahoo_youtube_uncertainties_seed{seed}_epoch{epoch}.pt')
...
yahoo_alpha_test = evaluate()
#yahoo_uncertainties(yahoo_alpha_test,0)
#amazon_uncertainties(0)
