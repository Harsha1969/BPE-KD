...
def parse_args():
    ...
    parser.add_argument(
        "--task_name",
        type=str,
        default='custom_amazon',
        help="The name of the glue task to train on, or 'custom_task'.",
    )
    parser.add_argument(
        "--train_file", type=str, default='train_amazon.csv', help="A csv or a json file containing the training data."
    )
    parser.add_argument(
        "--validation_file", type=str, default='test_amazon.csv', help="A csv or a json file containing the validation data."
    )
    parser.add_argument(
        "--max_length",
        type=int,
...
def main():
    ...
    # ## MODIFICATION START ##
    #
    # This entire block replaces your previous data loading logic.
    # It uses a robust, line-by-line reader to handle severe CSV errors.

    def robust_csv_load(filepath, expected_columns=3):
        """
        Reads a CSV file line-by-line, skipping any malformed rows.
        Returns a pandas DataFrame with only the valid rows.
        """
        clean_rows = []
        try:
            with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                for i, row in enumerate(reader):
                    # Only keep rows that have the exact number of expected columns
                    if len(row) == expected_columns:
                        clean_rows.append(row)
        except Exception as e:
            logger.error(f"Failed to read file {filepath}: {e}")
        
        column_names = ['rating', 'title', 'body']
        return pd.DataFrame(clean_rows, columns=column_names)


    if args.task_name == 'custom_amazon':
        ...
        logger.info("Starting robust CSV loading for the training file...")
        df_train_full = robust_csv_load(args.train_file, expected_columns=3)
        logger.info(f"Successfully loaded {len(df_train_full)} valid rows from training file.")

        logger.info("Starting robust CSV loading for the validation file...")
        df_validation_full = robust_csv_load(args.validation_file, expected_columns=3)
        logger.info(f"Successfully loaded {len(df_validation_full)} valid rows from validation file.")
        
        # Take the first 10,000 samples for the training set
        train_df = df_train_full.iloc[:10000]
        validation_df = df_validation_full[:5000]

        # Convert to a Hugging Face DatasetDict
    ...
    def preprocess_function(examples):
        if args.task_name == 'custom_amazon':
               # Combine title and body from your CSV into a single prompt.
               prompts = []
               for title, body in zip(examples["title"], examples["body"]):
                      prompt = f"Classify the sentiment of this Amazon review as 'positive' or 'negative'.\n\nTitle: {title}\nReview: {body}\nSentiment:"
                      prompts.append(prompt)
                               
               result = tokenizer(prompts, padding=padding, max_length=args.max_length, truncation=True)
                                                                               
               #Convert the 'rating' column (0 or 1) to integer labels.
               result["labels"] = [int(label) for label in examples["rating"]]
