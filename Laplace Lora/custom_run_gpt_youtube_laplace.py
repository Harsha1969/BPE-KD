...
def main(load_step):
    ...
    if args.task_name == 'custom_youtube':
        if not args.train_file or not args.validation_file:
            raise ValueError("--train_file and --validation_file must be provided for custom_youtube")
        
        train_df_full = pd.read_csv(args.train_file)
        validation_df_full = pd.read_csv(args.validation_file)
        train_df = train_df_full.iloc[:1100]
        validation_df = validation_df_full[1245:]
    ...
    class WrappedModel(torch.nn.Module):
        def __init__(self, model):
            super().__init__()
            if args.task_name == 'custom_youtube':
                not_spam_token_id = tokenizer.encode("not spam", add_special_tokens=False)[0]
                spam_token_id = tokenizer.encode("spam", add_special_tokens=False)[0]
                self.id_list = [not_spam_token_id, spam_token_id]
            else:
                self.id_list = None
            self.model = model
